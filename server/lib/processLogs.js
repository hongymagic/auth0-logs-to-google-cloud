...
module.exports = (storage) =>
  (req, res, next) => {
    ...
    const onLogsReceived = (logs, callback) => {
      ...
      logger.info(`Sending ${logs.length} logs to Google Cloud.`);

      const gclogging = new GoogleCloudLogging(config('GCLOUD_API_KEY'));
      gclogging.send(logs, callback);
    };

    const slack = new loggingTools.reporters.SlackReporter({
      hook: config('SLACK_INCOMING_WEBHOOK_URL'),
      username: 'auth0-logs-to-google-clou',
      title: 'Logs To Google Cloud'
    });
